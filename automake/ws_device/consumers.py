...
class DeviceConsumer(AsyncWebsocketConsumer):
    ...
    async def handle_report_material(self, payload):
        ...
        try:
            ...
            await database_sync_to_async(process_device_status_report)(self.sn, data)
            ...
            logger.error(f'[WS] 保存物料数据失败: sn={self.sn}, error={e}')
            await self.send_json({
                'type': 'error',
                'code': 'SERVER_ERROR',
                'msg_id': msg_id,
    ...
    async def handle_status_report(self, payload):
        ...
        try:
            ...
            logger.error(f'[WS] 处理设备状态上报失败: sn={self.sn}, error={e}')
            await self.send_json({
                'type': 'error',
                'code': 'SERVER_ERROR',
                'msg_id': msg_id,
                'message': f'设备状态处理失败: {str(e)}'
            })
