...
class OrderPrecheckView(APIView):
    ...
    def post(self, request):
        ...
        if not serializer.is_valid():
            logger.warning(f"用户 {request.user.id} 预校验订单参数错误: {serializer.errors}")
        ...
        logger.info(f"用户 {request.user.id} 预校验订单: store_id={store_id}, device_sn={device_sn}, items_count={len(items_data)}")

        try:
            result = precheck_order(store_id, items_data, device_sn=device_sn)
        except ValueError as e:
            logger.warning(f"用户 {request.user.id} 预校验失败: {e}")
        ...
        logger.debug(f"用户 {request.user.id} 预校验成功，返回支付金额: {result['pay_amount']}")
        return ok({
            'store_id': store_id,
...
class OrderCreateView(APIView):
    ...
    def post(self, request):
        serializer = CreateOrderSerializer(data=request.data)
        if not serializer.is_valid():
            logger.warning(f"用户 {request.user.id} 创建订单参数错误: {serializer.errors}")
            return error(str(serializer.errors), code=4001)
        ...
        logger.info(f"用户 {request.user.id} 发起创建订单: store_id={store_id}, device_sn={device_sn}")
        ...
        try:
            ...
            logger.warning(f"用户 {request.user.id} 创建订单业务校验失败: {e}")
            return error(str(e), code=4003)
        except Exception as e:
            logger.exception(f'创建订单系统异常: {e}')
            return error('创建订单失败，请稍后重试', code=4004, status=500)

        logger.info(f"用户 {request.user.id} 下单成功: order_no={order.order_no}, pay_amount={order.pay_amount}")
        return ok({
