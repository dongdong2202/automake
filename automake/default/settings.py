...
INSTALLED_APPS = [
    ...
    'monitor',                          # 设备监控与分析模块
    'admin_api',                        # 前端后台管理与数据分析 REST API
]
...
CORS_ALLOW_ALL_ORIGINS = DEBUG  # 开发模式下允许所有来源
...
REST_FRAMEWORK = {
    ...
    'DEFAULT_THROTTLE_RATES': {
        'anon': '100/day',
        'user': '1000/day',
        'order': '10/minute',  # Custom rate for ordering
    },
...
MIDDLEWARE = [
    'corsheaders.middleware.CorsMiddleware',         # 跨域中间件必须放在最顶部
...
# ============================================================
MQTT_HOST = os.environ.get('MQTT_HOST', 'tinylab.store')          # EMQX 服务地址
MQTT_PORT = int(os.environ.get('MQTT_PORT', '443'))          # MQTT 端口
...
LOGGING = {
    ...
    'loggers': {
        ...
        # 过滤客户端提前断开连接（如快速刷新/关闭页面/favicon请求）时的 asyncio 噪音日志
        'asyncio': {
            'handlers': ['console'],
            'level': 'CRITICAL',
            'propagate': False,
...
CELERY_BEAT_SCHEDULE = {
    # 每天清晨 08:30 自动全量扫描物料保质期（<=30天临期或过期自动生成平台告警并发送短信）
    'check_inventory_expiration_daily': {
        'task': 'inventory.tasks.check_inventory_expiration_task',
        'schedule': crontab(hour=8, minute=30),
    },
    # 每 5 分钟清理过期取餐码
    'expire_pickup_codes_every_5m': {
        'task': 'notifications.tasks.expire_pickup_codes',
        'schedule': 300.0,
    },
}
