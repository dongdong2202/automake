...
class InventoryRecordInline(TabularInline):
    ...
    fields = ('record_type', 'quantity', 'price', 'store', 'expiration_date', 'remarks', 'created_at', 'operator')
...
class MaterialAdmin(ModelAdmin):
    ...
    list_filter = ('material_type', 'storage_conditions')
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('code', 'name', 'price', 'unit', 'shelf_life', 'material_type')
        }),
        ('储存与属性', {
            'fields': ('storage_conditions', 'remarks')
        }),
        ('统计与账面 (只读，根据变动自动计算)', {
...
class InventoryRecordAdmin(ModelAdmin):
    ...
    fieldsets = (
        ('流向与类型', {
            'fields': ('material', 'record_type', 'quantity', 'price')
        }),
        ('出入库信息', {
