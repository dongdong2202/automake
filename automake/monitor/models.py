...
class DeviceMonitorSnapshot(models.Model):
    ...
    def check_health_status(self):
        """
        基于 raw_data 检查健康状况：
        优先使用 monitor.services.parse_device_status_payload
        """
        if not self.raw_data:
            return

        try:
            from monitor.services import parse_device_status_payload
            parsed = parse_device_status_payload(self.device_sn, self.raw_data)
            self.healthy = parsed.get('healthy', True)
            self.disconnected = parsed.get('disconnected', False)
            self.abnormality = parsed.get('abnormalities', {})
            if 'free' in parsed:
                self.mem_size = parsed['free']
        except Exception:
            # 基础后备逻辑
            raw_h = self.raw_data.get('healthy', 1)
            self.healthy = True if raw_h in (1, True, '1', 'true') else False
            raw_d = self.raw_data.get('disconnected', 0)
            self.disconnected = True if raw_d in (1, True, '1', 'true') else False

    def save(self, *args, **kwargs):
        if self.raw_data and not self.abnormality:
            self.check_health_status()
        if 'update_fields' in kwargs and kwargs['update_fields'] is not None:
            update_fields = set(kwargs['update_fields'])
            update_fields.add('healthy')
            update_fields.add('disconnected')
