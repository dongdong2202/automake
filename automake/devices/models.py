...
class Device(models.Model):
    ...
    province = models.CharField(max_length=64, blank=True, default='', db_index=True, verbose_name='省份')
    city = models.CharField(max_length=64, blank=True, default='', db_index=True, verbose_name='城市')
    address = models.CharField(max_length=256, blank=True, default='', verbose_name='设备地址')
    ...
    gps_coordinate = models.CharField(max_length=128, blank=True, default='', verbose_name='设备GPS坐标', help_text='例如：116.4074,39.9042')
...
class DeviceConsumableStock(models.Model):
    ...
    code = models.ForeignKey(
        ...
        limit_choices_to={'material_type__in': ['consumable', 'cup']}, verbose_name='耗材编码'
    )
...
class DeviceBarrelDict(models.Model):
    """
    料桶字典表
    记录桶编号、物料编号、设备编号、创建时间及创建人
    """
    barrel_code = models.CharField(
        max_length=64, db_index=True, verbose_name='桶编号', help_text='例如：b01, b02, b09 等'
    )
    material = models.ForeignKey(
        'inventory.Material', to_field='code', db_column='material_code',
        on_delete=models.CASCADE, related_name='barrel_dicts', verbose_name='物料编号'
    )
    device = models.ForeignKey(
    ...
    created_by = models.ForeignKey(
        ...
        null=True, blank=True, related_name='created_barrel_dicts', verbose_name='创建人'
    )
    ...
    def __str__(self):
        return f'{self.device_id} - {self.barrel_code}: {self.material_id}'


class DevicePoster(models.Model):
    """
    设备海报配置表
    单条海报配置支持同时上传横屏、竖屏、Banner 3个图标/图片，
    并支持多选指定适用门店与设备。
    """
    title = models.CharField(
        max_length=128, blank=True, verbose_name='海报配置名称',
        help_text='例如：秋季新品推广、默认轮播海报等'
    )
    
    # 3 个独立的上传图片字段
    horizontal_image = models.ImageField(
        upload_to='posters/horizontal/%Y/%m/',
        blank=True, null=True,
        verbose_name='横屏海报 (horizontal)',
        help_text='上传横屏海报图片'
    )
    vertical_image = models.ImageField(
        upload_to='posters/vertical/%Y/%m/',
        blank=True, null=True,
        verbose_name='竖屏海报 (vertical)',
        help_text='上传竖屏海报图片'
    )
    banner_image = models.ImageField(
        upload_to='posters/banner/%Y/%m/',
        blank=True, null=True,
        verbose_name='横幅海报 (banner)',
        help_text='上传Banner横幅图片'
    )
    ...
    stores = models.ManyToManyField(
        ...
        verbose_name='指定门店 (多选)',
        help_text='可多选适用门店；若不选门店且不选设备，则全局生效'
    )
    devices = models.ManyToManyField(
        'devices.Device',
        blank=True,
        related_name='posters',
        verbose_name='指定设备 (多选)',
        help_text='可多选适用设备'
    )

    version = models.IntegerField(
        unique=True, null=True, blank=True, verbose_name='版本号',
        help_text='整数版本号（唯一），留空时系统自动在当前最大版本基础上+1'
    )
    remarks = models.CharField(
        max_length=256, blank=True, default='', verbose_name='备注',
        help_text='例如：秋季开业海报、中秋限定海报等'
    )
    sort_order = models.IntegerField(default=0, verbose_name='排序权重', help_text='数值越小越靠前')
    ...
    class Meta:
        ...
        ordering = ['-version', 'sort_order', '-created_at']

    def save(self, *args, **kwargs):
        if self.version is None:
            max_v = DevicePoster.objects.aggregate(max_v=models.Max('version'))['max_v']
            self.version = (max_v or 0) + 1
        super().save(*args, **kwargs)
