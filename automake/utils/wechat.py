...
class WechatPayV3:
    ...
    def query_order(self, out_trade_no: str) -> dict:
        ...
        :return: 微信返回的订单状态字典 (包含 trade_state, transaction_id, amount 等)
        """
    ...
    def create_codepay_order(self, out_trade_no: str, amount: int, auth_code: str,
        ...
        key = getattr(settings, 'WECHAT_PAY_API_KEY', '') or self.api_v3_key
        ...
        string_a = '&'.join([f"{k}={params[k]}" for k in sorted_keys if params[k] != ''])
        ...
        try:
            resp = requests.post(url, data=xml_data.encode('utf-8'), headers={'Content-Type': 'text/xml'}, timeout=15)
            root = ET.fromstring(resp.content)
            res_dict = {child.tag: child.text for child in root}
            return res_dict
        except Exception as e:
            logger.error(f"付款码支付请求异常: {e}")
