...
from .models import (
    ...
    GlobalSkuTemplate, GlobalSkuTemplateIngredient,
    GlobalMenuSku, GlobalSkuIngredient
)


class GlobalSkuTemplateIngredientInline(TabularInline):
    """
    全局规格模板配料内联管理
    """
...
@admin.register(GlobalSkuTemplate)
class GlobalSkuTemplateAdmin(GlobalConfigAdmin):
    """
    全局规格模板（抽象规格库）后台管理
    """
    list_display = ('id', 'category', 'name', 'default_price_delta', 'is_active', 'sort_order', 'created_at')
    list_filter = ('is_active', 'category')
    search_fields = ('name', 'category', 'description')
    ordering = ('category', 'sort_order', 'id')
    inlines = [GlobalSkuTemplateIngredientInline]

    fieldsets = (
        ('基本信息', {
            'fields': ('category', 'name', 'default_price_delta', 'description')
        }),
        ('状态与排序', {
...
class GlobalMenuSkuAdmin(GlobalConfigAdmin):
    ...
    全局商品规格后台管理
    """
    list_display = ('id', 'item', 'template', 'price_delta', 'is_active', 'sort_order')
    list_filter = ('is_active', 'template__category', 'item__category')
    search_fields = ('template__name', 'template__category', 'item__name')
