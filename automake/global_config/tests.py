...
from .models import DeviceModel, GlobalMenuCategory, GlobalMenuItem, GlobalSkuTemplate, GlobalMenuSku, GlobalSkuIngredient
...
class GlobalMenuInheritanceTests(TestCase):
    def setUp(self):
        ...
        self.g_tpl_std = GlobalSkuTemplate.objects.create(
            name="标准", category="default", default_price_delta=0, is_active=True
        )
        self.g_sku_std = GlobalMenuSku.objects.create(
            item=self.g_item, template=self.g_tpl_std, price_delta=0, is_active=True
        )
        self.g_tpl = GlobalSkuTemplate.objects.create(
            name="大杯/热", category="杯型",
            default_price_delta=300, is_active=True
        )
    ...
    def test_local_menu_integrity_constraints(self):
        ...
        other_tpl = GlobalSkuTemplate.objects.create(
            name="规格", category="默认", default_price_delta=0, is_active=True
        )
        other_sku = GlobalMenuSku.objects.create(
            item=other_item, template=other_tpl, price_delta=0, is_active=True
        )
...
class GlobalMenuItemTests(TestCase):
    def test_no_auto_insert_standard_sku(self):
        ...
        # 验证不会自动创建规格，需手动从规格模板添加
        self.assertEqual(item.skus.count(), 0)
        
        tpl = GlobalSkuTemplate.objects.create(
            name="标准", category="default", default_price_delta=0
        )
        sku = GlobalMenuSku.objects.create(item=item, template=tpl)
        self.assertEqual(sku.name, '标准')
        self.assertEqual(sku.category, 'default')
        self.assertEqual(sku.price_delta, 0)
