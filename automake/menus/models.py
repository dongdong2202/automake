...
class MenuItem(models.Model):
    ...
    def sync_store_menu(cls, store):
        ...
        for g_item in global_items:
            ...
            # 同步对应的规格 (SKU)
            all_g_skus = g_item.skus.all()
            # 删除已被物理删除的全局规格关联
            MenuSku.objects.filter(item=menu_item).exclude(global_sku__in=all_g_skus).delete()

            for g_sku in all_g_skus:
                ...
                if menu_skus.exists():
                    ...
                    # 若全局规格已停用，强制设备端也停用
                    if not g_sku.is_active and menu_sku.is_active:
                        menu_sku.is_active = False
                        menu_sku.save(update_fields=['is_active'])
                else:
                    MenuSku.objects.create(
                        item=menu_item,
                        global_sku=g_sku,
                        price_delta=g_sku.price_delta,
                        is_active=False if not g_sku.is_active else True,
